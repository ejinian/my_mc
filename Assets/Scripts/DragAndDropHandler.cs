...
public class DragAndDropHandler : MonoBehaviour
{
    ...
    private void HandleSlotClick (UIItemSlot clickedSlot)
    {
        ...
        // cursor is empty but clicked slot has item
        if (!cursorSlot.HasItem && clickedSlot.HasItem)
        {
            cursorItemSlot.InsertStack(clickedSlot.itemSlot.TakeAll());
            return;
        }
        // cursor has item but clicked slot is empty, so put the item in the toolbar
        if (cursorSlot.HasItem && !clickedSlot.HasItem)
        {
            clickedSlot.itemSlot.InsertStack(cursorItemSlot.TakeAll());
            return;
        }
        // cursor has item and clicked slot has item, so swap items if different
        // if same then stack them
        if (cursorSlot.HasItem && clickedSlot.HasItem)
        {
